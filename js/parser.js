...
const parse = (tokens) => { //
  ...
  const parseExpression = () => {
    ...
    if (token.type === Types.NUMBER) { // Needs Types
      ...
      if (value.includes('/')) {
        ...
        return { type: Types.NUMBER, value: Fraction(numerator, denominator, true) }; // Needs Fraction
      } else {
        const numValue = parseFloat(value);
        if (isNaN(numValue)) {
          throw new Error(`Invalid number: ${value}`);
        }
        return { type: Types.NUMBER, value: Fraction(numValue, 1, false) }; // Needs Fraction
      }
    }

    if (token.type === Types.BOOLEAN) { // Needs Types
      const value = consume().value.toLowerCase();
      return { type: Types.BOOLEAN, value: value === 'true' };
    }

    if (token.type === Types.STRING) { // Needs Types
