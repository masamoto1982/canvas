const interpreter = (() => {
  ...
  const environment = {
    variables: {},
    functions: {},
    // Dictionary: 定義順を保持するための配列
    dictionary: []
  };

  // Dictionaryにワードを追加
  const addToDictionary = (word) => {
    if (!environment.dictionary.includes(word)) {
      environment.dictionary.push(word);
      // 長い順にソート（長いワードを優先的にマッチさせるため）
      environment.dictionary.sort((a, b) => b.length - a.length);
    }
  };

  // Dictionaryからワードを削除
  const removeFromDictionary = (word) => {
    const index = environment.dictionary.indexOf(word);
    if (index !== -1) {
      environment.dictionary.splice(index, 1);
    }
  };

  // Dictionaryを取得（tokenizer用）
  const getDictionary = () => {
      ...
      // Handle symbols: operators, keywords, functions, variables
        ...
        // Assignment
          ...
          if (value.type === 'quotation') {
            environment.functions[name] = value.value;
          } else {
        ...
        // FORGET command
          ...
          const nameToken = pop();
          if (nameToken.type !== Types.SYMBOL) throw new Error("FORGET requires a symbol.");
          const name = nameToken.value;
          
          // 変数と関数の両方から削除
          delete environment.variables[name];
          delete environment.functions[name];
          
          // Dictionaryから削除
          removeFromDictionary(name);
          
          console.log(`[DEBUG] Forgot word: "${name}"`);
        }
  ...
  return {
    ...
    getDictionary: getDictionary  // tokenizerから参照できるようにexport
  };
})();